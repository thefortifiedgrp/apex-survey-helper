...
export interface UseSurveyV2FlowOptions {
   ...
   *  patient-info form) when a returning/known customer's identity is already
   *  complete. Default OFF. Pass with `knownPatientInfo`. See engine docs. */
  skipPatientInfoWhenComplete?: boolean;
  /** Known identity for a returning customer on the drugIds path — seeds the
   *  engine so the skip guard can pass. Resolve it BEFORE calling this hook
   *  (the engine is built once from the options passed at that time). */
  knownPatientInfo?: Partial<PatientInfo>;
  /** Widen the fields required for the skip to fire (default: the engine's
   *  firstName/lastName/email/dob/state). Add any your downstream needs (e.g.
   *  shipping address) so the skip never drops them. */
